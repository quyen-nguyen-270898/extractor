...
public class WebServiceMain {
  ...
  private static void handleStreamByQuery(Context ctx) {
    ...
    try {
      ...
      // Detect common case where upstream returned an HTML bot-check/CAPTCHA
      if (msg != null && msg.contains("Got HTML document")) {
        // try to extract the latest attempted upstream URL if present in the
        // message
        String latest = null;
        final int idx = msg.indexOf("latest url was:");
        if (idx >= 0) {
          try {
            latest = msg.substring(idx + "latest url was:".length()).trim();
        ...
        final String hint = "Upstream responded with an HTML page (likely a bot-check/CAPTCHA). " +
            "Try setting UPSTREAM_PROXY to route requests through a different IP, or host the service on a different provider. " +
            "If you have a YouTube Data API key, set YT_API_KEY to reduce scraping.";

        var resp = new HashMap<String, String>();
        resp.put("error", "upstream returned HTML (bot-check)");
        resp.put("hint", hint);
        if (latest != null) resp.put("latest_upstream", latest);
        ctx.status(502).json(resp);
        return;
      }

      ctx.status(500).result("{\"error\":\"" + (msg != null ? msg.replaceAll("\"", "\\\"") : "unknown") + "\"}");
    }
  }
